...
static void test_uring_features(void **state) {
    ...
    if (features & IOR_FEAT_NATIVE_ASYNC) printf("  - Native async\n");
    if (features & IOR_FEAT_SPLICE) printf("  - Splice\n");
    if (features & IOR_FEAT_FIXED_FILE) printf("  - Fixed files\n");
    if (features & IOR_FEAT_POLL_ADD) printf("  - Poll add\n");
    if (features & IOR_FEAT_SQPOLL) printf("  - SQPOLL\n");
...
int main(void) {
    ...
    const struct CMUnitTest tests[] = {
        ...
        cmocka_unit_test_setup_teardown(test_uring_write, setup_uring_temp_file, teardown_temp_file),
        cmocka_unit_test_setup_teardown(test_uring_batch, setup_uring_temp_file, teardown_temp_file),
